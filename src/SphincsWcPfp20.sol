...
contract SphincsWcPfp20 {
    ...
    uint256 constant SUBTREE_H = 10;
    uint256 constant A = 12;
    uint256 constant K = 11;
    uint256 constant W = 32;
    uint256 constant L = 25;           // floor(128/5) = 25 message chains
    uint256 constant LEN1 = 25;        // floor(n_bits/log2(w)) = 25
    uint256 constant TARGET_SUM = 388; // optimal S_{w,n} for w=32, l=25
    uint256 constant Z = 0;
    // PORS+FP single-tree height: ceil(log2(k * 2^a)) = ceil(log2(11 * 4096)) = 16
    uint256 constant TREE_HEIGHT = 16;
    // Maximum Octopus auth nodes for k=11 in a height-16 tree.
    uint256 constant M_MAX = 98;
    ...
    function verify(bytes32 message, bytes calldata sig) external view returns (bool valid) {
        ...
        assembly ("memory-safe") {
            htIdx := and(shr(132, digest), sub(shl(20, 1), 1))
        }
