...
import "./SphincsSharedAccount.sol";

/// @title SphincsSharedFactory - Factory using a SHARED verifier for all accounts
...
contract SphincsSharedFactory {
    ...
    function createAccount(
        ...
        bytes32 pkSeed,
        bytes32 pkRoot
    ) external returns (SphincsSharedAccount account) {
        bytes32 salt = keccak256(abi.encodePacked(ecdsaOwner, pkSeed, pkRoot));

        account = new SphincsSharedAccount{salt: salt}(
    ...
    function getAddress(
        ...
        bytes32 hash = keccak256(abi.encodePacked(
            type(SphincsSharedAccount).creationCode,
