...
Full cycle: Type 1 register → Type 1 emergency → 58×Type 2 → Type 1 register
...
def jardin_type1_register(seed, sk, root, sub_pk, sub_root, r_slot, sig_hash):
...
def jardin_type2(sub_pk, sub_sk, sub_root, r_slot, q, fors_pks, spine, sent, sig_hash):
    fors_sig, _, _, _ = jardin_grind_and_sign(sub_pk, sub_sk, sub_root, sig_hash, q)
...
def encode_register_slot(r_slot, sub_pk, sub_root):
    """ABI-encode registerSlot(bytes32 r, bytes16 subSeed, bytes16 subRoot)."""
    from Crypto.Hash import keccak as _k
    h = _k.new(digest_bits=256)
    h.update(b"registerSlot(bytes32,bytes16,bytes16)")
    sel = h.digest()[:4]
    return (sel +
            r_slot.to_bytes(32, "big") +
...
def cmd_register(rpc, chain_id, sender, seed, sk, root, gen):
    ...
    sub_pk, sub_sk, sub_root, fps, sp, sn, r_slot = make_sub(sk, gen)
    res = do_tx(rpc, chain_id, sender, f"Type1 register (gen={gen})",
                lambda sh: jardin_type1_register(seed, sk, root, sub_pk, sub_root, r_slot, sh))
    return sub_pk, sub_sk, sub_root, fps, sp, sn, r_slot, res
...
def cmd_compact(rpc, chain_id, sender, sk, gen, q):
    ...
    return do_tx(rpc, chain_id, sender, f"Type2 q={q} (gen={gen})",
                 lambda sh: jardin_type2(sub_pk, sub_sk, sub_root, r_slot, q, fps, sp, sn, sh))
...
def cmd_full(rpc, chain_id, sender, seed, sk, root):
    ...
    reg1_data = encode_register_slot(r1, sub_pk, sub_root)
    _do("Type1 register (slot 1)",
        lambda sh: jardin_type1_register(seed, sk, root, sub_pk, sub_root, r1, sh),
        sender_data=reg1_data)

    # ── Emergency (no registration — SENDER frame is noop) ──
    print("\n========== EMERGENCY (r=0) ==========")
    _do("Type1 emergency (r=0)",
        lambda sh: jardin_type1_emergency(seed, sk, root, sh))

    # ── 58 compact sigs (VERIFY verifies FORS+C, SENDER is noop) ──
    print(f"\n========== SLOT 1: TYPE 2 q=1..{Q_MAX} ==========")
    for q in range(1, Q_MAX + 1):
        _do(f"Type2 q={q}",
            lambda sh, _q=q: jardin_type2(sub_pk, sub_sk, sub_root, r1, _q, fps, sp, sn, sh))

    # ── SLOT 2: Register ──
    print("\n========== SLOT 2: REGISTER ==========")
    sub_pk2, sub_sk2, sub_root2, _, _, _, r2 = make_sub(sk, 2)
    reg2_data = encode_register_slot(r2, sub_pk2, sub_root2)
    _do("Type1 register (slot 2)",
        lambda sh: jardin_type1_register(seed, sk, root, sub_pk2, sub_root2, r2, sh),
    ...
    if t2g:
        ...
        print(f"  q=1:  {t2g[0]}")
        if len(t2g) > 31: print(f"  q=32: {t2g[31]}")
